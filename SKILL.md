...
仅当华为云 Web/独立站部署需要图片素材时使用；必须走华为云 ModelArts MaaS API：`https://api.modelarts-maas.com/v1/images/generations`。默认模型为 `qwen-image`，也可用 `--model` 指定 MaaS 图片生成 API 支持且请求/响应结构兼容的 model 参数。脚本名和 manifest 默认名保留旧版兼容命名。详细流程见 `references/qwen-image-generation.md` 和 `references/playbooks/static-site-generated-assets-readiness.md`。
...
- 默认写入不含 API key、兼容旧名的 `qwen_manifest.json`，记录文件名、prompt、模型、endpoint host 和 created 时间
...
- 不把 MaaS 图像生成模型当作 `references/service-registry.json` 中的 KooCLI 云服务登记
